...
const Auditoria = () => {
  ...
  return (
    <Layout title="Auditoría" moduleIcon={<Activity/>}>
      ...
      {/* ✨ CONTROLES DE FECHA Y BÚSQUEDA (LIQUID GLASS) ✨ */}
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-4 md:mb-6 bg-white/60 dark:bg-blue-950/20 backdrop-blur-2xl p-3 md:p-4 rounded-[1.5rem] md:rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] border border-white/80 dark:border-white/5 gap-3 transition-colors duration-300">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 bg-white/50 dark:bg-transparent md:bg-transparent p-2 md:p-0 rounded-[1.25rem] md:rounded-none w-full xl:w-auto transition-colors">
          ...
          <div className="flex items-center justify-between bg-white/80 dark:bg-slate-900/50 backdrop-blur-md rounded-xl shadow-sm border border-gray-200/80 dark:border-white/10 transition-colors">
            ...
            <div className="px-2 min-w-[140px] md:min-w-[160px] text-center font-extrabold text-[10px] md:text-xs text-slate-800 dark:text-white tracking-widest uppercase flex items-center justify-center gap-1.5 transition-colors">
              <CalendarDays size={14} className="text-blue-600 dark:text-blue-400"/> {getLabelFecha()}
            </div>
          ...
          <div className="flex gap-1 bg-white/80 dark:bg-slate-900/50 sm:bg-transparent dark:sm:bg-transparent p-1 sm:p-0 rounded-xl sm:rounded-none border sm:border-0 border-gray-200/80 dark:border-transparent shadow-sm sm:shadow-none backdrop-blur-md transition-colors">
            <button onClick={() => {setViewMode('day'); setCurrentDate(new Date());}} className={`flex-1 sm:flex-none px-4 py-2 text-[10px] md:text-xs font-bold rounded-lg transition-colors ${viewMode === 'day' ? 'bg-blue-600/90 dark:bg-blue-600 text-white shadow-md shadow-blue-600/20' : 'text-slate-600 dark:text-slate-400 hover:bg-white/50 dark:hover:bg-slate-800'}`}>Día</button>
            <button onClick={() => setViewMode('month')} className={`flex-1 sm:flex-none px-4 py-2 text-[10px] md:text-xs font-bold rounded-lg transition-colors ${viewMode === 'month' ? 'bg-blue-600/90 dark:bg-blue-600 text-white shadow-md shadow-blue-600/20' : 'text-slate-600 dark:text-slate-400 hover:bg-white/50 dark:hover:bg-slate-800'}`}>Mes</button>
            <button onClick={() => setViewMode('all')} className={`flex-1 sm:flex-none px-4 py-2 text-[10px] md:text-xs font-bold rounded-lg transition-colors ${viewMode === 'all' ? 'bg-blue-600/90 dark:bg-blue-600 text-white shadow-md shadow-blue-600/20' : 'text-slate-600 dark:text-slate-400 hover:bg-white/50 dark:hover:bg-slate-800'}`}>Todo</button>
          </div>
        </div>
        ...
        <div className="relative w-full xl:w-72">
          ...
          <input 
            ...
            className="w-full bg-white/80 dark:bg-slate-900/50 backdrop-blur-md border border-gray-200/80 dark:border-white/10 pl-9 pr-3 py-2.5 rounded-xl text-xs md:text-sm outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white dark:focus:bg-blue-950 focus:border-blue-500 dark:focus:border-blue-400 font-bold text-gray-800 dark:text-white transition-all shadow-sm"
            value={searchUser} onChange={(e) => setSearchUser(e.target.value)}
          />
        </div>
      </div>

      {/* ✨ VISTA TÁCTIL (Móvil y Tablet hasta lg - LIQUID GLASS) ✨ */}
              ...
              <div key={u.id} className="bg-white/60 dark:bg-blue-950/20 backdrop-blur-xl rounded-[1.5rem] border border-gray-100/50 dark:border-white/5 p-4 shadow-sm relative group overflow-hidden transition-colors">
                 ...
                 <div className="bg-white/50 dark:bg-slate-900/50 p-2.5 rounded-xl border border-gray-100/50 dark:border-white/5 mb-3 grid grid-cols-2 gap-2 backdrop-blur-md transition-colors">
                    ...
                    <div className="flex flex-col items-end gap-1 overflow-hidden">
                      ...
                      <span className="text-[10px] font-black text-blue-700 dark:text-blue-400 bg-blue-50/80 dark:bg-blue-900/30 px-2 py-0.5 rounded-md border border-blue-100/50 dark:border-blue-500/20 backdrop-blur-md transition-colors">
      ...
      {/* ✨ VISTA PC (Tabla Liquid Glass - visible solo en monitores lg o superiores) ✨ */}
               ...
               usuariosMostrados.map((u) => (
                <tr key={u.id} className="hover:bg-white/40 dark:hover:bg-blue-900/10 transition-colors duration-200 group">
                  ...
                  <td className="px-6 py-4 text-center">
                     {u.total_acciones > 0 ? (
                       <span className="bg-blue-50/80 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 font-black px-3 py-1.5 rounded-lg text-[10px] border border-blue-100/50 dark:border-blue-500/20 backdrop-blur-md transition-colors">{u.total_acciones}</span>
                     ) : (
      ...
      {/* ✨ MODAL EXPEDIENTE (Bottom Sheet Móvil / Modal PC - LIQUID GLASS) ✨ */}
      {selectedUser && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center sm:p-4 bg-slate-900/40 dark:bg-slate-950/80 backdrop-blur-md transition-colors duration-300 animate-fade-in">
          <div className="bg-white/95 dark:bg-blue-950/90 backdrop-blur-3xl rounded-t-3xl sm:rounded-[2.5rem] w-full sm:max-w-5xl flex flex-col h-[90vh] sm:h-[85vh] overflow-hidden animate-fade-in-up border border-white/50 dark:border-white/10 shadow-2xl transition-colors">
            
            <div className="bg-slate-900 dark:bg-slate-950 p-5 md:p-6 flex justify-between items-center text-white shrink-0 relative overflow-hidden transition-colors">
               ...
               <div className="flex items-center gap-3 md:gap-4 z-10 mt-3 sm:mt-0">
                 ...
                 <div className="min-w-0">
                   <h2 className="text-base md:text-xl font-black tracking-tight truncate pr-4">Expediente: {selectedUser.nombre}</h2>
                   <p className="text-[9px] md:text-[10px] text-blue-300 font-extrabold tracking-widest uppercase mt-0.5 flex items-center gap-1"><CalendarDays size={10}/> {getLabelFecha()}</p>
                 </div>
               </div>
            ...
            <div className="bg-transparent border-b border-gray-100/50 dark:border-white/5 p-3 md:p-4 shrink-0 flex flex-col md:flex-row justify-between items-start md:items-center gap-3 transition-colors">
              <div className="relative w-full md:max-w-md">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-blue-400/70 transition-colors" size={16}/>
                <input 
                  type="text" placeholder="Buscar por módulo, acción o detalle..." 
                  className="w-full bg-white/80 dark:bg-slate-900/50 backdrop-blur-md border border-gray-200/80 dark:border-white/10 pl-9 pr-4 py-2.5 rounded-xl text-xs md:text-sm outline-none focus:bg-white dark:focus:bg-blue-950 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 dark:focus:border-blue-400 font-bold text-gray-800 dark:text-white transition-all shadow-sm"
                  value={searchDetail} onChange={(e) => setSearchDetail(e.target.value)}
                />
              </div>
              <div className="text-[10px] md:text-xs font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest bg-white/50 dark:bg-slate-900/50 backdrop-blur-md px-3 py-2 rounded-lg border border-gray-200/50 dark:border-white/5 self-end md:self-auto transition-colors">
                Mostrando: <span className="text-blue-600 dark:text-blue-400">{getLogsExpediente().length} Registros</span>
              </div>
            </div>

            {/* ✨ FIX: TIMELINE MÓVIL / TABLA PC ✨ */}
            <div className={`flex-1 overflow-y-auto bg-transparent relative ${hideScrollbar}`}>
              ...
              <div className="lg:hidden p-3 space-y-3">
                {getLogsExpediente().length === 0 ? (
                  ...
                  getLogsExpediente().map((log) => (
                    <div key={log.id} onClick={() => setLogDetail(log)} className="bg-white/60 dark:bg-slate-900/40 backdrop-blur-md p-3.5 rounded-[1.25rem] border border-gray-100/50 dark:border-white/5 shadow-sm flex flex-col gap-2.5 relative active:scale-[0.98] transition-transform">
                       <div className="flex justify-between items-start">
                         <div className="flex items-center gap-2 flex-wrap">
                           {getActionPill(log.accion)}
                           <span className="text-[9px] font-black uppercase text-slate-700 dark:text-slate-300 bg-white/80 dark:bg-slate-800/80 px-2 py-1 rounded-md border border-slate-200/50 dark:border-white/5 truncate max-w-[140px] backdrop-blur-md transition-colors">{log.modulo}</span>
                         </div>
                         <span className="text-[8px] font-extrabold text-slate-400 dark:text-slate-500 flex items-center gap-1 shrink-0 transition-colors"><Clock size={10}/> {formatearFecha(log.created_at, viewMode === 'day')}</span>
                       </div>
              ...
              {/* PC: Tabla (Liquid Glass) */}
              <div className="hidden lg:block w-full h-full">
                <table className="w-full text-left text-sm text-gray-600 dark:text-slate-300 border-collapse transition-colors">
                  <thead className="sticky top-0 z-30 bg-white/90 dark:bg-blue-950/90 backdrop-blur-xl shadow-sm transition-colors">
                    <tr>
                      <th className="px-6 py-4 border-b border-gray-100/50 dark:border-white/5 text-[9px] uppercase font-black text-gray-400 dark:text-blue-300/70 tracking-widest w-[15%]">Hora</th>
                      <th className="px-6 py-4 border-b border-gray-100/50 dark:border-white/5 text-[9px] uppercase font-black text-gray-400 dark:text-blue-300/70 tracking-widest w-[15%]">Módulo</th>
                      <th className="px-6 py-4 border-b border-gray-100/50 dark:border-white/5 text-[9px] uppercase font-black text-gray-400 dark:text-blue-300/70 tracking-widest w-[15%]">Acción</th>
                      <th className="px-6 py-4 border-b border-gray-100/50 dark:border-white/5 text-[9px] uppercase font-black text-gray-400 dark:text-blue-300/70 tracking-widest w-[45%]">Vista Previa</th>
                      <th className="px-6 py-4 border-b border-gray-100/50 dark:border-white/5 text-[9px] uppercase font-black text-gray-400 dark:text-blue-300/70 tracking-widest w-[10%] text-center">Detalle</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50/50 dark:divide-white/5 relative z-0 bg-transparent transition-colors">
                    {getLogsExpediente().length === 0 ? (
                      <tr><td colSpan="5" className="text-center py-12 text-gray-400 dark:text-slate-500 italic font-bold text-xs transition-colors">No se encontraron registros.</td></tr>
                    ) : (
                      getLogsExpediente().map((log) => (
                        <tr key={log.id} className="hover:bg-white/50 dark:hover:bg-slate-900/50 transition-colors group cursor-pointer" onClick={() => setLogDetail(log)}>
                          ...
                          <td className="px-6 py-4 font-black text-slate-700 dark:text-slate-300 text-xs transition-colors">{log.modulo}</td>
                          ...
                          <td className="px-6 py-4 text-center">
                            <button className="p-2 text-blue-500 dark:text-blue-400 bg-white/80 dark:bg-slate-800/80 hover:bg-blue-50 dark:hover:bg-slate-700 rounded-lg transition-colors border border-gray-200/50 dark:border-white/5 shadow-sm backdrop-blur-md active:scale-95" title="Ver Detalle Técnico">
                              <Eye size={16}/>
                            </button>
                          </td>
                        </tr>
            ...
            {/* Botón de cierre para móvil (Fijo abajo) */}
            <div className="p-4 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl border-t border-gray-100/50 dark:border-white/5 sm:hidden shrink-0 transition-colors">
               <button onClick={() => setSelectedUser(null)} className="w-full bg-white/80 dark:bg-slate-800/80 border border-gray-200/80 dark:border-white/5 hover:bg-gray-100 dark:hover:bg-slate-700 text-gray-700 dark:text-slate-300 py-3.5 rounded-xl font-extrabold text-sm shadow-sm transition-colors active:scale-95">Cerrar Expediente</button>
            </div>
          </div>
        </div>
      )}
      ...
      {logDetail && (
        <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center sm:p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-md transition-colors animate-fade-in">
          <div className="bg-white/95 dark:bg-blue-950/90 backdrop-blur-3xl rounded-t-3xl sm:rounded-[2rem] shadow-2xl w-full sm:max-w-md p-6 sm:p-8 border border-white/50 dark:border-white/10 animate-fade-in-up relative pb-8 sm:pb-8 flex flex-col max-h-[85vh] transition-colors">
            <div className="w-12 h-1.5 bg-gray-200 dark:bg-slate-700 rounded-full mx-auto mb-4 sm:hidden shrink-0"></div>
            <button onClick={() => setLogDetail(null)} className="absolute top-4 right-4 sm:top-5 sm:right-5 bg-gray-50/50 dark:bg-slate-800/50 text-gray-400 dark:text-slate-500 hover:bg-gray-100 dark:hover:bg-slate-700 p-2 rounded-full transition-colors border border-transparent dark:border-white/5"><X size={16}/></button>
            
            <div className="shrink-0">
              <h3 className="text-[10px] font-extrabold text-gray-400 dark:text-blue-300/70 uppercase tracking-widest mb-1 border-b border-dashed border-gray-200/50 dark:border-slate-700 pb-2 transition-colors">Reporte Técnico</h3>
              <div className="flex items-center gap-2 mt-4 mb-5 flex-wrap">
                 {getActionPill(logDetail.accion)}
                 <span className="text-[10px] font-black uppercase text-slate-700 dark:text-slate-300 bg-white/80 dark:bg-slate-800/80 px-2 py-1 rounded-md border border-slate-200/50 dark:border-white/5 backdrop-blur-md transition-colors">{logDetail.modulo}</span>
              </div>
            </div>
            
            <div className={`bg-white/50 dark:bg-slate-900/50 p-4 sm:p-5 rounded-2xl border border-gray-200/50 dark:border-white/5 shadow-inner overflow-y-auto ${hideScrollbar} flex-1 backdrop-blur-md transition-colors`}>
               <p className="text-[11px] sm:text-xs font-bold text-slate-700 dark:text-slate-300 leading-relaxed break-words transition-colors">
                 {logDetail.detalles}
               </p>
            </div>
