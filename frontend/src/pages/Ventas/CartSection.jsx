...
/**
 * Carrito: lista de ítems con +/- y eliminar, subtotal, IGV, total.
 * Combobox de cliente y botón Finalizar Venta.
 */
const CartSection = ({
  items,
  onUpdateQty,
    ...
    <div className="flex flex-col h-full bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="flex-1 overflow-auto p-4 space-y-2">
        {items.length === 0 ? (
          ...
          items.map((it) => (
            <div
              key={it.id + (it.codigo || '')}
              className="flex items-center gap-2 p-2 border border-gray-100 rounded-lg bg-gray-50/50"
            >
      ...
      <div className="p-4 border-t border-gray-200 space-y-2 bg-gray-50">
        ...
        <button
          ...
          className="w-full mt-3 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-lg transition-colors"
        >
          Finalizar Venta
        </button>
      </div>
