...
const DirectorioAdmin = () => {
  ...
  const validarFormulario = () => {
    ...
    if (formData.telefono && !/^[\d\s\-\+]{9,15}$/.test(formData.telefono.replace(/\s/g, ''))) {
      nuevosErrores.telefono = "Ingrese un número de teléfono válido (mín. 9 dígitos).";
    }
    
    if (formData.correo_personal && !/^\S+@\S+\.\S+$/.test(formData.correo_personal)) {
      nuevosErrores.correo_personal = "El formato del correo no es válido.";
    }
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (modalEditar) {
        await api.put(`/empleados/${modalEditar.id}`, formData);
      } else {
    ...
    <Layout>
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1 font-medium">Gestiona la información personal y confidencial de los colaboradores.</p>
        </div>
