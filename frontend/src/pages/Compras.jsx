...
const Compras = () => {
  ...
  return (
    <Layout title="Compras" moduleIcon={<ShoppingCart/>}>
      ...
      {/* ✨ FIX: Vista Táctil (Móvil y Tablet) organizada en 2 columnas ✨ */}
              ...
              <div key={compra.id} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm relative group overflow-hidden">
                 <div className="flex justify-between items-start mb-3 border-b border-dashed border-gray-100 pb-3">
                    <div className="flex items-center gap-3">
                       ...
                       <div>
                         <p className="font-extrabold text-gray-800 text-xs md:text-sm truncate max-w-[150px] md:max-w-[200px]">{compra.proveedor_nombre || 'Proveedor S/D'}</p>
                         <p className="text-[9px] md:text-[10px] font-bold text-gray-400 flex items-center gap-1 mt-0.5">{new Date(compra.created_at).toLocaleDateString('es-PE', {day:'2-digit', month:'short', year:'2-digit', hour:'2-digit', minute:'2-digit'})}</p>
                       </div>
                    </div>
                    <div className="text-right">
                      <span className="bg-emerald-50 text-emerald-600 border border-emerald-100 px-1.5 py-0.5 rounded text-[8px] md:text-[9px] font-black uppercase tracking-wider mb-1 inline-block">{compra.estado}</span>
                      <p className="font-black text-gray-800 text-sm md:text-base leading-none">S/ {parseFloat(compra.total).toFixed(2)}</p>
                    </div>
                 </div>

                 <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100 mb-3 flex items-center gap-2">
                    <FileText size={12} className="text-gray-400 shrink-0"/>
                    <span className="text-[10px] md:text-[11px] font-bold text-gray-600 truncate">Doc: {compra.comprobante || 'S/C'}</span>
                 </div>

                 {esVistaGlobal && (
                   <div className="mb-3">
                     <span className="inline-flex items-center gap-1 text-[9px] md:text-[10px] font-bold text-purple-700 bg-purple-50 border border-purple-100 px-2 py-1 rounded w-fit">
      ...
      {/* ✨ FIX: Vista PC (Tabla) visible solo en pantallas muy grandes (lg) ✨ */}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/60 backdrop-blur-md p-0 md:p-4">
          <div className="bg-white md:rounded-[2rem] w-full h-full md:h-auto md:max-h-[90vh] md:max-w-5xl shadow-2xl flex flex-col animate-fade-in-up overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto flex flex-col md:flex-row bg-slate-50/30">
               ...
               <div className="flex-1 flex flex-col border-r border-gray-100 relative">
                 ...
                 <div className="p-3 md:p-4 bg-white border-b border-gray-100 shrink-0 relative z-10 shadow-sm">
                   <div className="relative">
                     <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                     <input 
                       type="text" 
                       placeholder="Buscar por Nombre o SKU..." 
                       className="w-full bg-slate-50 border border-slate-200 pl-9 pr-8 py-2.5 md:py-3 rounded-xl outline-none focus:bg-white focus:ring-2 focus:ring-blue-100 focus:border-blue-400 transition-all font-bold text-gray-800 text-xs md:text-sm" 
                       value={busquedaProd} 
                       onChange={(e) => setBusquedaProd(e.target.value)} 
                     />
                     {busquedaProd && (
                       <button onClick={() => setBusquedaProd('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 bg-gray-100 rounded-full p-0.5">
                         <X size={14}/>
                       </button>
                     )}
                   </div>
                   ...
                   {busquedaProd && (
                     <div className="absolute left-3 right-3 md:left-4 md:right-4 mt-2 bg-white border border-gray-200 rounded-xl shadow-2xl z-30 max-h-[35vh] overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                       {productosFiltrados.length === 0 ? (
                         <div className="p-6 text-center flex flex-col items-center">
                           <Package size={24} className="text-gray-300 mb-2"/>
                           <p className="text-gray-500 text-[10px] md:text-[11px] font-bold">No hay coincidencias en esta sede.</p>
                         </div>
                       ) : (
                         productosFiltrados.map(prod => (
                           <div key={prod.id} onClick={() => agregarAlCarrito(prod)} className="p-2.5 md:p-3 border-b border-gray-50 hover:bg-blue-50 cursor-pointer flex items-center gap-3 transition-colors group">
                             <div className="w-8 h-8 md:w-10 md:h-10 bg-slate-50 rounded-lg flex items-center justify-center shrink-0 border border-slate-100 overflow-hidden">
                                {prod.imagen ? <img src={renderImagen(prod.imagen)} className="w-full h-full object-cover"/> : <ImageIcon size={14} className="text-gray-300"/>}
                             </div>
                             <div className="flex-1 min-w-0">
                               <p className="font-bold text-[11px] md:text-xs text-gray-800 group-hover:text-blue-700 truncate transition-colors leading-tight">{prod.nombre}</p>
                               <p className="text-[8px] md:text-[9px] font-bold text-gray-400 uppercase tracking-widest mt-0.5 truncate">{prod.codigo || 'S/C'}</p>
                             </div>
                 ...
                 <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    {cart.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-full min-h-[150px] md:min-h-[200px]">
                        <div className="w-14 h-14 md:w-16 md:h-16 bg-slate-100 rounded-full flex items-center justify-center mb-3 border-[3px] border-white shadow-sm">
                           <Package size={20} className="text-slate-300 md:w-6 md:h-6" />
                        </div>
                        <p className="text-[11px] md:text-xs font-bold text-gray-600">Lista de ingreso vacía</p>
                        <p className="text-[9px] md:text-[10px] text-gray-400 mt-1 text-center max-w-[200px]">Busca productos arriba para agregarlos a la compra.</p>
                      </div>
                    ) : (
                      cart.map((item, index) => (
                        <div key={index} className="bg-white p-2.5 md:p-3 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-2 relative group overflow-hidden">
                           
                           <div className="flex items-center gap-2">
                             <div className="w-8 h-8 md:w-10 md:h-10 bg-slate-50 rounded-lg flex items-center justify-center shrink-0 border border-slate-100 overflow-hidden">
                                {item.imagen ? <img src={renderImagen(item.imagen)} className="w-full h-full object-cover"/> : <ImageIcon size={12} className="text-gray-300"/>}
                             </div>
                             <div className="flex-1 min-w-0 pr-6">
                               <p className="font-bold text-gray-800 text-[10px] md:text-xs leading-tight line-clamp-2">{item.nombre}</p>
                               <p className="text-[8px] font-black text-gray-400 uppercase mt-0.5">{item.codigo || 'S/C'}</p>
                             </div>
                             
                             <button onClick={() => eliminarDelCarrito(index)} className="absolute top-2 right-2 p-1.5 text-red-400 hover:text-red-600 bg-red-50/50 hover:bg-red-50 rounded-lg transition-colors shrink-0">
                           ...
                           <div className="flex items-center gap-2 mt-1 border-t border-gray-50 pt-2">
                              <div className="flex flex-col flex-1">
                                <label className="text-[8px] font-bold text-gray-400 uppercase ml-0.5 block mb-0.5">Cantidad</label>
                                <div className="flex items-center bg-slate-50 border border-slate-200 rounded-lg p-0.5 w-full justify-between">
                                   <button type="button" onClick={() => actualizarItem(index, 'cantidad', Math.max(1, item.cantidad - 1))} className="w-6 h-6 flex items-center justify-center rounded-md bg-white text-gray-500 shadow-sm hover:text-blue-600 active:scale-90 transition-all shrink-0"><Minus size={12}/></button>
                                   <input type="number" min="1" className="w-8 md:w-10 text-center font-bold text-[10px] md:text-xs text-gray-800 bg-transparent outline-none [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" value={item.cantidad} onChange={(e) => actualizarItem(index, 'cantidad', e.target.value)} />
                                   <button type="button" onClick={() => actualizarItem(index, 'cantidad', item.cantidad + 1)} className="w-6 h-6 flex items-center justify-center rounded-md bg-white text-gray-500 shadow-sm hover:text-blue-600 active:scale-90 transition-all shrink-0"><Plus size={12}/></button>
                                </div>
                              </div>
                              ...
                              <div className="flex flex-col flex-1">
                                ...
                                <div className="flex items-center bg-slate-50 border border-slate-200 rounded-lg focus-within:border-blue-400 focus-within:bg-white transition-colors overflow-hidden">
                                  <span className="pl-1.5 text-gray-400 font-bold text-[9px] md:text-[10px]">S/</span>
                                  <input 
                                    type="number" min="0" step="0.01" 
                                    className="w-full py-1.5 md:py-2 pl-1 pr-2 outline-none font-bold text-gray-800 text-[10px] md:text-xs bg-transparent [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" 
                           ...
                           <div className="mt-1 text-right border-t border-dashed border-gray-100 pt-1">
                             <p className="font-black text-emerald-600 text-[10px] md:text-[11px]">Subtotal: S/ {item.subtotal.toFixed(2)}</p>
                           </div>
                        </div>
                      ))
                    )}
               ...
               <div className="w-full md:w-[300px] lg:w-[340px] bg-white border-t md:border-t-0 md:border-l border-gray-100 flex flex-col shrink-0">
                 <div className="p-4 md:p-5 space-y-3 md:space-y-4 flex-1 overflow-y-auto">
                   <div>
                     <label className="text-[9px] font-extrabold text-blue-600 uppercase tracking-widest block mb-1 flex items-center gap-1 bg-blue-50 w-fit px-1.5 py-0.5 rounded"><Building2 size={10}/> Proveedor *</label>
                     <select className="w-full bg-slate-50 border border-slate-200 p-2.5 rounded-xl outline-none focus:bg-white focus:border-blue-400 font-bold text-gray-700 text-[11px] md:text-sm transition-colors" value={proveedorId} onChange={(e) => setProveedorId(e.target.value)}>
                   ...
                   <div>
                     <label className="text-[9px] font-extrabold text-blue-600 uppercase tracking-widest block mb-1 flex items-center gap-1 bg-blue-50 w-fit px-1.5 py-0.5 rounded"><FileText size={10}/> Comprobante</label>
                     <div className="relative">
                        <FileText className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" size={14} />
                        <input type="text" placeholder="Ej: F001-000234" className="w-full bg-slate-50 border border-slate-200 p-2.5 pl-8 rounded-xl outline-none focus:bg-white focus:border-blue-400 font-bold text-gray-800 uppercase transition-colors text-[11px] md:text-sm" value={comprobante} onChange={(e) => setComprobante(e.target.value)} />
                     </div>
                   </div>
                 </div>

                 <div className="p-4 md:p-5 bg-slate-50 border-t border-gray-100 shrink-0 pb-6 md:pb-5">
                   <div className="bg-slate-900 p-3 md:p-4 rounded-xl border border-slate-800 shadow-md text-white text-center relative overflow-hidden mb-3">
                     <div className="absolute -left-4 -bottom-4 w-16 h-16 bg-emerald-500 opacity-20 rounded-full blur-2xl pointer-events-none"></div>
                     <p className="text-[8px] md:text-[9px] font-extrabold text-slate-400 uppercase tracking-widest mb-0.5 relative z-10">Costo Total a Pagar</p>
                     <p className="text-xl md:text-2xl font-black text-emerald-400 tracking-tight relative z-10">S/ {calcularTotal().toFixed(2)}</p>
                   </div>
                   
                   <button onClick={handleSubmit} disabled={cart.length === 0} className="w-full bg-blue-600 disabled:bg-slate-300 disabled:text-gray-500 text-white py-3 md:py-3.5 rounded-xl font-black text-xs md:text-sm hover:bg-blue-700 shadow-lg shadow-blue-600/30 disabled:shadow-none transition-all active:scale-[0.98] flex items-center justify-center gap-1.5">
